...
## 2. Пользовательские сценарии и кейсы использования
...
### 2.3. Альтернативные и исключительные сценарии
...
### 2.4. Диаграммы прецедентов (Use-case diagrams)
...
## 3. Функциональные блоки (высокоуровневая структура)

### 3.1. Ингестинг документов
...
### 3.3. Разбиение на чанки / сегментация
...
### 3.5. Хранилище векторов (Vector DB)
...
### 3.6. Механизм поиска / ранжирования
...
### 3.8. Модель генерации (LLM) и промпты
...
### 3.10. Управление сессиями и контекстом
...
#### 4.2.1. Поддержка многоязычности

Русский, английский.

#### 4.2.2. Обработка изображений/сканов

В первой версии не реализуется.

#### 4.2.3. Обнаружение макета / таблиц

Не используется в v1, предусмотрено архитектурно.

### 4.3. Чанкинг и аннотации

#### 4.3.1. Алгоритмы разбиения
...
#### 4.5.2. Индексация и настройки поиска

HNSW + quantization (опционально).
Топология: single-node Qdrant.

#### 4.5.3. Репликация и шардирование

В первой версии не используется, допускается добавление.

### 4.6. Retrieval / Reranking

#### 4.6.1. Стратегии поиска
...
#### 4.6.3. Ограничение по количеству чанков

top_m = 5-12 (после rerank).

### 4.7. RAG и генерация ответа

#### 4.7.1. Интерфейс к LLM

Async API, поддержка streaming.

#### 4.7.2. Шаблоны промптов
...
### 4.8. Диалоги и контекст сессии

#### 4.8.1. Хранение истории

5 последних сообщений сохраняются на диск.

#### 4.8.2. Контекстная сегментация
...
REST + WebSocket/SSE (stream output).

#### 4.9.3. Админ интерфейс
...
**TL;DR** - по ощущениям.
#### 5.1.1. Целевые задержки (latency) для поиска и генерации
#### 5.1.2. Пропускная способность (throughput)

### 5.2. Точность и качество (quality of answers)
#### 5.2.1. Оценка Embeddings
...
#### 5.2.3. Оценка Reranking (опционально)
